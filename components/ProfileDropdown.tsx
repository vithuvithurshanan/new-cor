...
export const ProfileDropdown: React.FC<ProfileDropdownProps> = ({ user, onProfileClick, onLogout }) => {
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="flex items-center gap-3 p-2 hover:bg-white/50 rounded-xl transition-all duration-200 w-full"
      >
        <div className="w-8 h-8 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center font-bold ring-2 ring-white overflow-hidden">
          {user.avatarUrl ? (
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute bottom-full left-0 right-0 mb-2 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/50 z-50 overflow-hidden animate-in slide-in-from-bottom-2 duration-200">
            {/* User Info Header */}
            <div className="p-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-purple-50">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center font-bold text-lg overflow-hidden">
                  {user.avatarUrl ? (
                    <img src={user.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    user.name.charAt(0)
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs text-slate-500 capitalize">{user.status.toLowerCase()}</span>
                  </div>
                </div>
              </div>
            </div>
